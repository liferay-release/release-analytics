...
# 2b. LOAD STORIES
# =============================================================================
stories_path <- file.path(STAGING, "raw_jira_stories.rds")
stories_raw  <- NULL
if (file.exists(stories_path)) {
  stories_raw <- readRDS(stories_path)
  message(sprintf("  Stories loaded: %d issues", nrow(stories_raw)))
} else {
  message("  staging/raw_jira_stories.rds not found — story_count will not be updated")
  message("  Run extract_jira.R first to populate stories")
}

# =============================================================================
# 2c. LOAD BREAKING CHANGES
# Breaking changes CSV: data/breaking_changes_YYYYMMDD.csv
# Columns expected: module_path, quarter, breaking_count
# Generated by git extraction script (searches for #breaking in commit messages)
# =============================================================================
breaking_raw <- NULL
breaking_files <- sort(Sys.glob("data/breaking_changes_*.csv"), decreasing = TRUE)
if (length(breaking_files) > 0) {
  breaking_file <- breaking_files[1]  # most recent
  breaking_raw  <- read_csv(breaking_file, col_types = cols(.default = "c"), show_col_types = FALSE) |>
    mutate(breaking_count = as.integer(breaking_count))
  message(sprintf("  Breaking changes loaded: %d rows from %s", nrow(breaking_raw), breaking_file))
} else {
  message("  No data/breaking_changes_*.csv found — breaking_count will not be updated")
  message("  Run git extraction script to populate breaking changes")
}
...
# Helper: assign story quarter — use fixVersion if available, else dev window
story_to_quarter <- function(quarter_col, quarter_source_col, created_dates, windows) {
  mapply(function(q, src, cd) {
    if (!is.na(q) && src == "fixVersion") return(q)
    # Fall back to created_date dev window lookup
    date_to_dev_quarter(cd, windows)
  }, quarter_col, quarter_source_col, created_dates, USE.NAMES = FALSE)
}
...
# --- Stories ---
story_counts <- NULL
if (!is.null(stories_raw) && nrow(stories_raw) > 0) {
  stories_assigned <- stories_raw |>
    mutate(
      quarter = story_to_quarter(quarter, quarter_source, created_date, dev_windows)
    ) |>
    filter(!is.na(quarter), quarter >= "2024.Q1")
  
  message(sprintf("  Stories: %d assigned to quarters, %d excluded",
                  nrow(stories_assigned),
                  nrow(stories_raw) - nrow(stories_assigned)))
  
  # Expand components (same logic as LPP/LPD)
  stories_expanded <- stories_assigned |>
    rowwise() |>
    mutate(component_list = list(parse_jira_components(components))) |>
    ungroup() |>
    unnest(component_list) |>
  ...
  stories_matched <- stories_expanded |>
    left_join(dim_comp, by = "component_name") |>
    filter(!is.na(component_id))
  
  story_counts <- stories_matched |>
    group_by(component_id, quarter) |>
    summarise(story_count = n(), .groups = "drop")
  
  message(sprintf("  Story rollup: %d component × quarter rows", nrow(story_counts)))
}

# --- Breaking changes ---
breaking_counts <- NULL
if (!is.null(breaking_raw) && nrow(breaking_raw) > 0) {
  # Join module_path → component via dim_module_component_map
  mcm <- dbGetQuery(con, "
    SELECT mcm.module_path, mcm.component_id, mcm.weight
    FROM dim_module_component_map mcm
  ")
  
  breaking_mapped <- breaking_raw |>
    left_join(mcm, by = "module_path") |>
    filter(!is.na(component_id), !is.na(quarter), quarter >= "2024.Q1") |>
    group_by(component_id, quarter) |>
    summarise(
      breaking_count = as.integer(round(sum(as.numeric(breaking_count) * weight))),
      .groups = "drop"
    )
  
  message(sprintf("  Breaking changes rollup: %d component × quarter rows", nrow(breaking_mapped)))
  breaking_counts <- breaking_mapped
}
...
if (!is.null(story_counts) && nrow(story_counts) > 0) {
  upsert_bug_counts(story_counts, "story_count", con)
} else {
  message("  story_count: skipped (no data)")
}

if (!is.null(breaking_counts) && nrow(breaking_counts) > 0) {
  upsert_bug_counts(breaking_counts, "breaking_count", con)
} else {
  message("  breaking_count: skipped (no data)")
}

# -----------------------------------------------------------------------------
# Fix is_forecast_row — set TRUE for IN_DEVELOPMENT quarter
# -----------------------------------------------------------------------------
forecast_rows_fixed <- dbExecute(con, "
  UPDATE fact_forecast_input fi
  SET is_forecast_row = TRUE
  WHERE fi.quarter IN (
    SELECT release_label FROM dim_release
    WHERE release_status = 'IN_DEVELOPMENT'
  )
  AND fi.is_forecast_row IS DISTINCT FROM TRUE;
")
message(sprintf("  is_forecast_row fixed: %d rows set to TRUE", forecast_rows_fixed))
...
summary_q <- dbGetQuery(con, "
  SELECT
    ...
    SUM(fi.story_count)     AS total_stories,
    SUM(fi.breaking_count)  AS total_breaking,
